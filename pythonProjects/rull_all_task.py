...
str_crontab = os.popen("crontab -l").readlines()
if DEBUG:
...
for line in str_crontab.split('\n'):
    ...
    if 'otask' in line:
        ...
        global_crontab_task.append("otask {}" + line.split('otask')[-1])
        count += 1
...
if __name__ == "__main__":
    ...
    for i in range(count):
        pool.apply(func, (global_crontab_task[i],))  # 维持执行的进程总数为processes，当一个进程执行完毕后会添加新的进程进去
