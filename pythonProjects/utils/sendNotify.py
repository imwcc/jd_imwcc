...
class   sendNotify:
    ...
    PUSH_PLUS_TOKEN_ADMIN = readSecret("PUSH_PLUS_TOKEN_ADMIN")

    # def __init__(self, token):
    #     self.PUSH_PLUS_TOKEN = token

    def set_push_push_token(self, token):
        self.PUSH_PLUS_TOKEN = token
    ...
    def pushPlusAdminNotify(self, text, desp):
        if self.PUSH_PLUS_TOKEN_ADMIN != '':
            desp = re.sub('[\n\r]', '<br>', desp, 0)
            response = json.dumps(requests.post('http://www.pushplus.plus/send', data=json.dumps({
                'token': self.PUSH_PLUS_TOKEN, 'title': text, 'content': desp, 'topic': self.PUSH_PLUS_TOKEN_ADMIN}),
                                                headers={'Content-Type': 'application/json;charset=utf-8'}).json(),
                                  ensure_ascii=False)
            print(response)
